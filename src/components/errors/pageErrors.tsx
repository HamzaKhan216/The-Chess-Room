...
function PageError(props: { title: string, description?: string, errorKey: number, removeError: (errorKey: number) => void, newErrorKey: number, y: string, type: PageErrorProps['type'] }) {
    ...
    return (
        <div ref={errorRef} data-errorkey={errorKey} style={{opacity: hided ? 0 : 100, backgroundColor: type === 'error' ? 'var(--error)' : type === 'warning' ? 'var(--warning)' : type === 'success' ? 'var(--highlightBrilliant)' : ''}} className="p-3 text-xl select-text z-[999] text-white font-bold rounded-borderRoundness hover:scale-105 will-change-transform transition-all max-w-96">
            { title }
